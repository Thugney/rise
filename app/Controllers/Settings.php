...
class Settings extends Security_Controller {
    ...
    function ai_settings() {
        ...
        $view_data['total_subscriptions'] = $Ai_subscriptions_model->count_all_where(array('deleted' => 0));
        $view_data['active_subscriptions'] = $Ai_subscriptions_model->count_all_where(array('status' => 'active', 'deleted' => 0));

        return $this->template->rander("settings/ai_settings", $view_data);
    }
    ...
    function save_ai_settings() {
        ...
        foreach ($settings as $setting) {
            ...
            if (in_array($setting, array('ai_max_tokens', 'ai_rate_limit_per_hour'))) {
                $value = (int) $value;
                if ($value < 1) {
                    $value = $setting === 'ai_max_tokens' ? 4096 : 60;
                }
            }

            // Validate temperature
            if ($setting === 'ai_temperature') {
                $value = (float) $value;
                if ($value < 0 || $value > 2) {
                    $value = 0.7;
                }
    ...
    function test_ai_connection() {
        ...
        $response = $Ai_deepseek->query("Say 'Connection successful!' in exactly those words.");
        ...
        if (isset($response['error'])) {
            ...
            echo json_encode(array(
                "success" => true,
                "message" => "Connected successfully to " . $Ai_deepseek->get_model(),
    ...
    function test_polar_connection() {
        ...
        if (!$Polar->is_configured()) {
            echo json_encode(array(
                "success" => false,
                "message" => "Polar.sh not configured. Please enter API token and product ID."
            ));
            return;
        }
        ...
        echo json_encode(array(
            "success" => true,
            "message" => "Polar.sh configuration looks valid. Webhook URL: " . get_uri('polar_webhook')
        ));
    }
    ...
    function ai_analytics() {
        ...
        $stats = array(
            'total_queries' => $Ai_conversations_model->count_all_where(array('deleted' => 0)),
            'queries_today' => $Ai_conversations_model->get_queries_count_since(date('Y-m-d 00:00:00')),
            'active_users' => $Ai_conversations_model->get_active_users_count(30), // Last 30 days
            'total_tokens' => $Ai_conversations_model->get_total_tokens(),
            'total_subscriptions' => $Ai_subscriptions_model->count_all_where(array('deleted' => 0)),
            'active_subscriptions' => $Ai_subscriptions_model->count_all_where(array('status' => 'active', 'deleted' => 0)),
            'canceled_subscriptions' => $Ai_subscriptions_model->count_all_where(array('status' => 'canceled', 'deleted' => 0)),
        );

        // Calculate conversion rate (active/total * 100)
        $stats['conversion_rate'] = $stats['total_subscriptions'] > 0
            ? ($stats['active_subscriptions'] / $stats['total_subscriptions']) * 100
            : 0;

        // Get top users
        $view_data['top_users'] = $Ai_conversations_model->get_top_users(10);

        // Get recent conversations
        $view_data['recent_conversations'] = $Ai_conversations_model->get_recent_conversations(20);

        // Get subscribers list
        $view_data['subscribers'] = $Ai_subscriptions_model->get_subscribers_with_usage();

        // Get usage chart data (last 30 days)
        $view_data['usage_chart_data'] = $Ai_conversations_model->get_daily_usage_stats(30);

        $view_data['stats'] = $stats;

        return $this->template->rander("settings/ai_analytics", $view_data);
    }

    /**
     * Export AI conversation logs
     */
    function export_ai_logs() {
        $this->access_only_admin();

        $Ai_conversations_model = model("App\Models\Ai_conversations_model");
        $conversations = $Ai_conversations_model->get_all_for_export();

        $filename = "ai_conversation_logs_" . date('Y-m-d_His') . ".csv";
        ...
        // CSV headers
        fputcsv($output, array('ID', 'User', 'Session ID', 'Query', 'Response', 'Tokens', 'Date'));
